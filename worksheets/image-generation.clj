...
(def clipart (Clipart. WIDTH HEIGHT)) ; Renderer object

(defn render [xs ys letters & {:keys [mode] :or {mode Clipart/RELATIVE}}]
  (.background clipart)
...
(defn render-to-file [xs ys letters filename & {:keys [mode] :or {mode Clipart/RELATIVE}}]
  (.background clipart)
  (.text clipart (char-array letters) (int-array xs) (int-array ys) mode)
  (.blurGaussian clipart 2 2.0)
...
;(render x-offsets y-offsets letters false :mode OxCaptcha/RELATIVE)

(render-to-file x-offsets y-offsets letters "file.png")
...
;; @@
;; Copy and paste of the captcha example
...
;; @@
;; =>
;;; {"type":"html","content":"<span class='clj-var'>#&#x27;exercises.captcha/smc-captcha-MAP-state</span>","value":"#'exercises.captcha/smc-captcha-MAP-state"}
;; <=
