...
public class Clipart {
    ...
    public Clipart background() {
        return background(_bg_color);
    }
    public Clipart background(Color color) {
        _bg_color = color;
    ...
    public Clipart foreground(Color color) {
        _fg_color = color;
        return this;
    }
    ...
    public Clipart text(int length) {
    ...
    public Clipart text(String chars) {
        return text(chars, (int)(0.05 * _width), (int)(0.75 * _height), 0);
    }

    public Clipart text(String chars, int kerning) {
        return text(chars, (int)(0.05 * _width), (int)(0.75 * _height), kerning);
    }

    public Clipart text(char[] chars) {
        return text(chars, (int)(0.05 * _width), (int)(0.75 * _height), 0);
    }

    public Clipart text(String chars, int xOffset, int yOffset, int kerning) {
        int l = chars.length();
        char[] t = new char[l];
    ...
    public Clipart text(char[] chars, int xOffset, int yOffset, int kerning) {
    ...
    public Clipart text(char[] chars, int[] xOffsets, int[] yOffsets) {
        return text(chars, xOffsets, yOffsets, Clipart.RELATIVE);
    }

    // Add letters with either RELATIVE (per letter) or ABSOLUTE positioning.
    // if mode is Clipart.ABSOLUTE, xs and ys are absolute positions of letters in chars
    // if mode is Clipart.RELATIVE, xs and ys are relative positions of letters in chars:
    // Offsets give the position of each letter relative to the top right of the previous letter
    // The offsets of the first letter are relative to the top left of the image
    // (For an image 50 pixels high, it's a good idea to start the first y offset around 30, so that the text is inside the image)
    // x increases from left to right
    // y increases from top to bottom
    public Clipart text(char[] chars, int[] xs, int[] ys, int mode) {
        if (mode == Clipart.RELATIVE) {
            _xOffsets = xs;
            _yOffsets = ys;
        } else {
            _xs = xs;
    ...
    public Clipart blur(int kernelSize) {
    ...
    public Clipart blurGaussian(int radius, double sigma) {
        if (radius < 1) {
    ...
    public Clipart blurGaussian3x3() {
    ...
    public Clipart blurGaussian5x5s1() {
    ...
    public Clipart blurGaussian5x5s2() {
    ...
    public Clipart noiseCurvedLine() {
        return noiseCurvedLine(_fg_color, 3.0f);
    }

    public Clipart noiseCurvedLine(Color color, float thickness) {
    ...
    public Clipart noiseStraightLine() {
        return noiseStraightLine(_fg_color, 3.0f);
    }

    public Clipart noiseStraightLine(Color color, float thickness) {
    ...
    public Clipart noiseSaltPepper() {
        return noiseSaltPepper(0.01f, 0.01f);
    }

    public Clipart noiseSaltPepper(float salt, float pepper) {
    ...
    public Clipart distortion() {
        return distortionShear();
    }

    public Clipart distortionFishEye() {
...
//        }
    ...
    public Clipart distortionShear() {
    ...
    public Clipart distortionShear(int xPeriod, int xPhase, int yPeriod, int yPhase) {
        shearX(_img_g, xPeriod, xPhase, _width, _height);
        shearY(_img_g, yPeriod, yPhase, _width, _height);
        return this;
    }

    public Clipart normalize() {
